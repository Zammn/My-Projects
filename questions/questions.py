...
def tokenize(document):
    ...
    word_list = [word for word in nltk.word_tokenize(document.lower()) if word.isalpha() and not word in stopwords]
    return word_list
    ...
    for sentence in sentences:
        ...
        for word in query:    
            if word in sentences[sentence]:
                ...
                points[sentence] = (sent_idf, w_in_sent / len(sentences[sentence]))
    sent_sorted = [sent for sent, value in sorted(points.items(), key=lambda item: (-item[1][0], -item[1][1]))]
