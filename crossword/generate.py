...
class CrosswordCreator():
            ...
            if word not in assignment:
                ...
                for n in neighbors:
                    if word in self.domains[n]:
                        removed += 1
                collection[word] = removed
        return sorted(collection, key=lambda key: collection[key])
    ...
    def select_unassigned_variable(self, assignment):
        ...
        result_variable = unassigned[0]
        for variable in unassigned:
            if result_variable != variable:
                if domain(result_variable) > domain(var):
                    result_variable = var
                elif domain(result_variable) == domain(var):
                    if degree(var) >= degree(result_variable):
                        result_variable = var
